...
| Per-command argument validators (`git push`, `find -delete`, `tar -c`, publish flags, …) | audit only | enforced | enforced |
...
3. **Argument validation**: per-command validators block dangerous flags (e.g. `find -exec`, `tar -c`, `git push`). Write commands (`cp`, `mv`, `rm`, `sed`, etc.) are allowed but their paths are validated. `tar -x` and `unzip` may extract: the working directory and any `-C`/`--directory` or `-d` destination must be writable, and the options that run programs (`--to-command`, `--use-compress-program`, `-I`, ...) or let entries escape the destination (`-P`, `--overwrite`, `unzip -:`, ...) are blocked, as are `TAR_OPTIONS`, `TAPE`, `UNZIP`, and `UNZIPOPT`. The entries inside an archive are kept under the destination by the tools' own defaults (leading `/` and `..` stripped, no writing through symlinks the archive creates); the OS sandbox is what confines them outright. `zip` accepts only a fixed set of options (`-r`, `-q`, `-0`..`-9`, `-j`, `-y`, `-x`, `-i`, `-u`, `-d`, `-T`, ...; not `-TT`, `-@`, `-m`, `-b`, `-O`), and the archive it writes must be writable; `ZIPOPT` and `ZIP` are blocked.
