...
1. **Command deny list**: invocations matching a denied command (a `commands` entry with `allow: false`, or a built-in entry for the sandbox's own `config`, `install`, `update`, and `hook` subcommands) are refused before any other command gate, in both `denylist` and `allowlist` modes. No allowed command, `no_sandbox` or not, lifts this check. It is re-applied at the runtime layer and inside command wrappers. See [Denied commands](configuration.md#denied-commands).
