...
| Command deny list (`denied_commands` + the built-in self-protection entries) | audit only | enforced | enforced |
...
read-only in `denylist` mode — is enabled. `denied_commands` extends the list;
see [Denied commands](configuration.md#denied-commands).

## Static preflight (AST-level, before execution)

1. **Command deny list** — Invocations matching `denied_commands` (which includes built-in entries for the sandbox's own `config`, `install`, `update`, and `hook` subcommands) are refused before any other command gate, in `denylist` mode as well as `allowlist`. This is the one check `extra_commands` and `unsandboxed_commands` do not lift, and it is re-applied at the runtime layer and inside command wrappers. See [Denied commands](configuration.md#denied-commands).
...
- **The deny list is a gate, not a boundary**: `denied_commands` is enforced at the same validation layers as everything else, so it stops an agent from *invoking* a denied command — not a program that is already running from doing the same work itself. With the OS sandbox off, a denied `lite-sandbox config` does not stop `perl -e` from writing the config file directly, because no path argument ever reaches the AST layer; the OS sandbox's read-only mount of the config and the agents' settings (`denylist` mode) is what closes that, and the two are meant to be used together.
