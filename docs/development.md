...
Before any test runs, regardless of `-run`, `TestMain` sets up everything in `e2e/mockedserver/.bin` (override with `E2E_BIN_DIR`). It builds `lite-sandbox`, downloads the pinned Crush, Codex, and opencode GitHub releases, downloads the pinned Claude Code native binary from the distribution `claude.ai/install.sh` uses, verified against its manifest checksum, and downloads the pinned Grok Build binary from the distribution `x.ai/cli/install.sh` uses, verified against a SHA-256 pinned in `versions.go` (Grok publishes no checksum). No Node or npm is involved. If `GITHUB_TOKEN` (or `GH_TOKEN`) is set, as it is in the workflow, GitHub downloads go through the authenticated REST API and avoid the per-IP rate limit; otherwise they use the public download URLs.

Versions are pinned in `e2e/mockedserver/versions.go`. Each agent is installed into its own versioned directory (`e2e/mockedserver/.bin/agents/<agent>/<version>`), so switching versions never re-downloads one that's already there. To try a different version for one run, set `E2E_CRUSH_VERSION`, `E2E_CODEX_VERSION`, `E2E_CLAUDE_CODE_VERSION`, `E2E_OPENCODE_VERSION`, or `E2E_GROK_VERSION`. Without `LITE_SANDBOX_E2E`, every test skips, so `go test ./...` stays offline. The `E2E` GitHub workflow runs the same command on Linux and macOS, caching `e2e/mockedserver/.bin/agents` keyed on `versions.go` and `main_test.go`, so CI behaves the same as a local run.
...
Each test isolates its agent in per-test temp directories using the agent's own config-dir variables (`CRUSH_GLOBAL_CONFIG`, `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `GROK_HOME`, and the XDG directories for opencode). The installers honor the same variables, so nothing on the developer's machine is read or written.

#### The mock model

The agents use three wire protocols: Crush, opencode, and Grok (configured with a custom `chat_completions` model) use OpenAI chat completions, Codex uses only the OpenAI Responses API, and Claude Code uses the Anthropic Messages API. `mockmodel` is one server with three endpoints (`/v1/chat/completions`, `/v1/responses`, `/v1/messages`, streaming and non-streaming). Tests don't see the protocol: every request is normalized into a `Turn` (the tool names offered and the tool results so far), and the scripted behavior is the same for all of them: make the next scripted tool call, or give the final answer once every call has a result. Codex exposes MCP tools under a namespace (`mcp__lite_sandbox` with functions `bash`, …), which the mock records as `mcp__lite_sandbox__bash` and calls back with the namespace-qualified form. Grok offers no MCP tools directly: the mock calls its `use_tool` dispatcher with `tool_name` `lite-sandbox__bash`. Grok names each session through a separate request offering a lone `session_title` tool, which the mock treats as a side request (`Script.SideTools`).
...
- **Grok Build**: default (the hook redirects `run_terminal_command` and denies a `read_file` outside the project, then `use_tool` runs the scenario, which headless `grok -p` only allows because of the installer's permission rule) and `--bash-ast-hook-mode` (the hook rejects `curl`, lets `echo` run, and still denies the out-of-bounds read).
