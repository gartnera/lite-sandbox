...
- The tag is created by the workflow through the GitHub API (a lightweight tag on the released commit), then GoReleaser builds against it and uploads to a **draft** release (`release.draft: true`). The workflow then attests the assets and publishes the draft as its last step, so a release is never visible half-built. If a run fails after the tag exists, re-running the workflow redoes that release — the leftover draft is replaced (`replace_existing_draft`) rather than reused, and the version script only counts a *published* release as done — as long as `main` hasn't moved on; once it has, the next release starts from the orphaned tag, which keeps whatever the failed run left behind (nothing, or an unpublished draft release a maintainer can delete).
- **Immutable releases.** Enable *release immutability* for the repository once, under *Settings → General → Releases* (or `gh api -X PUT repos/gartnera/lite-sandbox/immutable-releases`; it needs admin, which the workflow token does not have). From then on a published release's assets can't be added, changed, or removed and its tag can't be moved or deleted, and GitHub signs a release attestation for the assets on publish. That is why everything that touches assets happens while the release is a draft: GoReleaser refuses to update an immutable release, and the workflow prints a warning if a release came out mutable (the setting is off). Releases published before the setting was enabled stay mutable.
- **Attestations.** `actions/attest` signs a [SLSA build-provenance attestation](https://docs.github.com/en/actions/concepts/security/artifact-attestations) for each `*.tar.gz` and for `checksums.txt` through GitHub's Sigstore instance (the job has `id-token: write` and `attestations: write`). Attestations are stored on the repository, not as release assets, so the asset list `lite-sandbox update` relies on is unchanged. Verify one with `gh attestation verify <asset> -R gartnera/lite-sandbox`.
