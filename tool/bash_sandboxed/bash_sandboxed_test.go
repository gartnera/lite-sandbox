...
func TestValidate_AllowedCommands(t *testing.T) {
	tests := []struct {
		...
		{"tar list checkpoint", "tar -tf archive.tar --checkpoint=100"},
		{"tar list attached archive", "tar -tfarchive.tar"},
		{"tar extract", "tar -xf archive.tar"},
		{"tar extract gz", "tar -xzf archive.tar.gz"},
		{"tar extract verbose to dir", "tar -xvf archive.tar -C out"},
		{"tar extract attached dir", "tar -xf archive.tar -Cout"},
		{"tar extract long", "tar --extract --file archive.tar --directory=out"},
		{"tar extract abbreviated", "tar --ext --file=archive.tar --dir out"},
		{"tar extract get", "tar --get -f archive.tar"},
		{"tar extract bsdtar cd", "tar -xf archive.tar --cd out"},
		{"tar extract strip", "tar -xf archive.tar --strip-components=1"},
		{"tar extract stdin", "tar -xzf - < archive.tar.gz"},
		{"tar extract stdout", "tar -xOf archive.tar member"},
		{"tar extract overwrite dir", "tar -xf archive.tar --overwrite-dir"},
		{"tar old style extract", "tar xf archive.tar"},
		{"tar old style extract gz to dir", "tar xzfC archive.tar.gz out"},
		{"tar extract local colon name", "tar -xf ./a:b.tar"},
		...
		{"unzip extract quiet overwrite", "unzip -qo archive.zip"},
		{"unzip extract to dir", "unzip archive.zip -d out"},
		{"unzip extract attached dir", "unzip -dout archive.zip"},
		{"unzip extract members", "unzip archive.zip 'src/*' -x '*.bak'"},
		{"unzip to stdout", "unzip -p archive.zip file.txt"},
		{"unzip with password", "unzip -P secret archive.zip"},
