...
Drives the real Crush, Codex, and Claude Code binaries through `lite-sandbox install <agent>` and a non-interactive run, with an in-process mock standing in for the model. Run it with `LITE_SANDBOX_E2E=1 go test ./e2e/mockedserver/ -v`.
...
- Each test isolates its agent in temp directories via `CRUSH_GLOBAL_CONFIG`, `CODEX_HOME`, or `CLAUDE_CONFIG_DIR`, and runs offline (`HTTPS_PROXY` points at a closed port).
