...
- monty (embedded Python) comes from `github.com/fugue-labs/monty-go`, replaced in `go.mod` with the `gartnera/monty-go` fork. The fork commits `monty.wasm` and embeds it, so no Rust toolchain is needed — `go build` is all it takes.
