...
- `config/` — YAML config loading, watching, and per-directory overrides (any section, via `Config.ForDirectory`); `mode.go` holds the mode enum and the built-in deny lists. Every `config` subcommand inherits one persistent `--dir` flag (registered on `configCmd`): `loadConfig` hands the command the config that directory resolves to and `saveConfig` records the sections it changed as a directory override, so individual commands need no per-directory code
